...
pdf("../output/Figure 1.pdf", width = 8, height = 4)
...
pdf("../output/Figure 2.pdf", width = 12, height = 3.5)
...
s <- c("LIHC", "PRAD")
#
# tiff("../output/Figure 3.tiff", width = 10, height = 3, units = "in", res = 600,
#      compression = "lzw")

pdf("../output/Figure 3.pdf", width = 10, height = 3)
...
#
# tiff("../output/Figure 4.tiff", width = 9, height = 9, units = "in", res = 600,
#      compression = "lzw")

pdf("../output/Figure 4.pdf", width = 9, height = 9)

par(mfrow = c(4, 4), oma = c(1, 1, 1, 1), mar = c(4, 4, 2, 1))
...
plotAdaptiveWeights(0.016, n = 200, type = "line",ylim = c(0, 0.045))
mtext(bquote(kappa ~ "= 0.016 (optimized)"), side = 3, line = 0.2, adj = 0)
mtext("BRCA", side = 2, line = 3.5)

plotAdaptiveWeights(0.01, n = 200, type = "line",ylim = c(0, 0.04))
mtext(bquote(kappa ~ "= 0.01"), side = 3, line = 0.2, adj = 0)

plotAdaptiveWeights(0.03, n = 200, type = "line",ylim = c(0, 0.04))
mtext(bquote(kappa ~ "= 0.03"), side = 3, line = 0.2, adj = 0)

plotAdaptiveWeights(0.05, n = 200, type = "line",ylim = c(0, 0.04))
mtext(bquote(kappa ~ "= 0.05"), side = 3, line = 0.2, adj = 0)
...
pdf("../output/Figure 5.pdf", width = 10, height = 3 * 3)
...
tiff("../output/Figure A1.tiff", width = 10, height = 6 * 3, units = "in",
     res = 300, compression = "lzw")
...
################################################################################
#- Comparison: unweighted correlations, weighted kendal, and adaptive
...
pdf("../output/Figure 6.pdf", width = 12, height = 3 * 2.5)
...
tiff("../output/Figure A4.tiff", width = 12, height = 6 * 2, units = "in",
     res = 300, compression = "lzw")
...
pdf("../output/Figure 7.pdf", width = 10, height = 2.5)
...
################################################################################
# Explore the source of influence
################################################################################
#
# library(dendextend)
# ## load LUSC data
...
# tiff("../output/Figure 8.tiff", width = 8, height = 8, units = "in", res = 600,
#      compression = "lzw")

pdf("../output/Figure 8.pdf", width = 8, height = 8)

par(mfrow = c(4, 2), mar = c(1.5, 1, 1.5, 1), oma = c(1, 1, 1, 1))
...
legend("topright", legend = c("Cancer", "Normal"),
       col = c("darkcyan", "darkgoldenrod1"), lty = 1, cex = 0.8)
...
clusterPlot("PRAD", 107)
mtext("G, PRAD", side = 3, adj = 0)

clusterPlot("THCA", 134)
mtext("H, THCA", side = 3, adj = 0)

dev.off()


#  boxplot, distribution of tumor genes ----------------------------------------

boxPlotFun <- function(datName, obs) {

    load("../output/drop1out.ranks.RData")
    obj <- drop1out.ranks[[datName]]
    top200 <- obj$origRank
    ...
    # import oncoGenes
    # https://www.oncokb.org/cancer-genes
    geneList <- read.csv("./cancerGeneList.tsv", sep = "\t")
    geneList <- geneList[, 1]
    topOncoGenes <- intersect(top200, geneList)[1:5]
    Dat <- apply(X[topOncoGenes, ], 1, scale)
    ...
    Dat <- Dat %>%
        ...
        mutate(grp = y,
               ind = row_number()) %>%
        tidyr::pivot_longer(cols = seq_len(ncol(Dat)))


    point_data <- Dat %>% filter(ind == obs)

    mv <- ifelse(y[obs] == "01", -0.2, 0.2)
    col <- ifelse(y[obs] == "01", "darkcyan", "darkgoldenrod1")

    # grouped boxplot
    ggplot(Dat, aes(x = name, y = value, fill = grp)) +
        geom_boxplot(outliers  = FALSE) +
        scale_fill_manual(breaks = c("01", "11"),
        ...
        geom_point(data = point_data,
                   ...
                   colour = col,
                   size = 1.5,
                   position = position_nudge(x = mv)) +
        theme_bw() +
        theme(legend.position = "bottom",
              legend.title = element_blank(),
              axis.title.x = element_blank(),
              axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
        ggtitle(paste0(datName, "(obs", obs, ")"))

}

p1 <- boxPlotFun("LUSC", obs = 51)
p2 <- boxPlotFun("STAD", obs = 22)
p3 <- boxPlotFun("BRCA", obs = 338)
p4 <- boxPlotFun("COAD", obs = 73)
p5 <- boxPlotFun("HNSC", obs = 93)
p6 <- boxPlotFun("KIRC", obs = 133)
p7 <- boxPlotFun("PRAD", obs = 107)
p8 <- boxPlotFun("THCA", obs = 134)

p <- ggpubr::ggarrange(p1, p2, p3, p4, p5, p6, p7, p8, ncol = 2, nrow = 4,
                       legend = "bottom",
                       common.legend = TRUE)
ggsave("../output/Figure A5.tiff", plot = p, width = 6, height = 8, units = "in",
       compression = "lzw")

# ggsave("../output/Figure A5.jpeg", plot = p, width = 6, height = 8,
#        units = "in")


################################################################################
# Figure 9, A4: Effects of IPs on GSEA
# Note, this figure may be different because of the update of org.Hs.eg.db database
################################################################################
...
# tiff("../output/Figure 9.tiff", width = 8, height = 6, units = "in", res = 600,
#      compression = "lzw")

pdf("../output/Figure 9.pdf", width = 8, height = 6)
...
tiff("../output/Figure A6.tiff", width = 8, height = 9, units = "in", res = 600,
     compression = "lzw")
...
#  cluster, using tumor genes ----------------------------------------
#
# clusterTumorPlot <- function(datName, obs) {
#
#     library(dendextend)
#     ## load LUSC data
#
...
#     # https://www.oncokb.org/cancer-genes
#     geneList <- read.csv("./cancerGeneList.tsv", sep = "\t")
#     geneList <- geneList[geneList$OncoKB.Annotated == "Yes", 1]
#     topOncoGenes <- intersect(top200, geneList)
#     Z <- apply(X[topOncoGenes, ], 1, scale)
#
#     d <- stats::dist(Z)
#     h <- hclust(d)
#
...
#
#
# tiff("../output/Figure tumorCluster.tiff", width = 8, height = 8, units = "in", res = 600,
#      compression = "lzw")
#
# jpeg("../output/Figure tumorCluster.jpeg", width = 8, height = 8, units = "in", res = 600)
#
# par(mfrow = c(4, 2), mar = c(1.5, 1, 1.5, 1), oma = c(1, 1, 1, 1))
#
# clusterTumorPlot("LUSC", 51)
# mtext("A, LUSC", side = 3, adj = 0)
#
# clusterTumorPlot("STAD", 22)
# mtext("B, STAD", side = 3, adj = 0)
#
# legend("topright", legend = c("Cancer", "Normal"),
#        col = c("darkcyan", "darkgoldenrod1"), lty = 1, cex = 0.8)
#
# clusterTumorPlot("BRCA", 338)
# mtext("C, BRCA", side = 3, adj = 0)
#
# clusterTumorPlot("COAD", 73)
# mtext("D, COAD", side = 3, adj = 0)
#
# clusterTumorPlot("HNSC", 93)
# mtext("E, HNSC", side = 3, adj = 0)
#
# clusterTumorPlot("KIRC", 133)
# mtext("F, KIRC", side = 3, adj = 0)
#
# clusterTumorPlot("PRAD", 107)
# mtext("G, PRAD", side = 3, adj = 0)
#
# clusterTumorPlot("THCA", 134)
# mtext("H, THCA", side = 3, adj = 0)
#
#
# dev.off()
...
load("../output/drop1out.ranks_limma.RData")
tiff("../output/Figure A2.tiff", width = 12, height = 5, units = "in",
     res = 600, compression = "lzw")
...
# output influential plot
load("../output/drop1out.ranks_kruskal.RData")
